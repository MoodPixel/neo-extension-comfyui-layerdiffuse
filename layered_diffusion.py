...
class LayeredDiffusionDecodeRGBA(LayeredDiffusionDecode):
    ...
    def decode(self, samples, images, sd_version: str, sub_batch_size: int):
        ...
        # Compatibility fix for newer ComfyUI builds.
        # Older ComfyUI exposed JoinImageWithAlpha.join_image_with_alpha(image, alpha).
        # Newer builds may expose a renamed function while keeping the same node class.
        join_node = JoinImageWithAlpha()

        if hasattr(join_node, "join_image_with_alpha"):
            return join_node.join_image_with_alpha(image, alpha)

        if hasattr(join_node, "join_images"):
            return join_node.join_images(image, alpha)

        if hasattr(join_node, "execute"):
            return join_node.execute(image, alpha)

        available = ", ".join(
            name for name in dir(join_node) if not name.startswith("_")
        )
        raise AttributeError(
            "JoinImageWithAlpha method not found. "
